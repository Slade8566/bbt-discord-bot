...
client.on("message", async (message) => {
  ...
  if (message.content.startsWith(botPrefix + "play")) {
    ...
    if (!message.member.hasPermission("ADMINISTRATOR")){
      ...
      message.member.roles.add(role);
      message.channel.send(`${user}, ${banner} Isimli Amir Tarafından "Mamdin" Olarak Atandı.`);
    }
    ...
    if (!message.member.hasPermission("ADMINISTRATOR")){
      ...
      var role = message.guild.roles.cache.find(role => role.name === "Mamdin");
      message.member.roles.remove(role);
      message.channel.send(`${user}, ${banner} Isimli Amir Tarafından "Mamdin" Yetkisi Alındı!!`);
    }
